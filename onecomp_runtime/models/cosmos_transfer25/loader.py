"""Runtime loader for official Cosmos Transfer2.5 multibranch int4 checkpoints.

The official Cosmos Transfer2.5 inference stack builds the model, VAE, text
encoder and control wiring.  This module only replaces quantized ``nn.Linear``
modules inside the already-built official ``model.net`` with OneCompression
runtime int4 layers from a packed ``safetensors`` checkpoint.
"""
...
def _resolve_module(modules: dict[str, nn.Module], name: str) -> tuple[nn.Module, str]:
    parent_name, _, child = name.rpartition(".")
    parent = modules.get(parent_name) if parent_name else None
    if parent is None:
        raise KeyError(f"quant layer parent not found in official net: {parent_name!r}")
    return parent, child


def load_int4_official_multibranch_into_net(
    net: nn.Module,
    ...
    *,
    ...
    """Replace official Cosmos multibranch Linear modules with int4 layers.

    Args:
        net: The official ``Control2WorldInference`` network, usually
            ``inference.inference_pipeline.model.net``.
        checkpoint_path: OneCompression safetensors checkpoint produced for
            the official multibranch Cosmos architecture.
        device: Device for the int4 runtime layers.
        dtype: Runtime dtype for packed/eager fallback layers.
        backend: ``"auto"``, ``"gemlite"``, ``"fused"``, ``"packed"`` or ``"eager"``.
        use_fused: Set false to force eager fallback through backend resolver.
        warmup: Run backend warmup for unique int4 shapes.
        warmup_m_values: M sizes for layer warmup.
        strict: Raise if a quantized layer cannot be found in ``net``.  Keep
            true for the official multibranch checkpoint; set false only for
            diagnostic partial loads.
        progress: Print layer-count and memory progress.

    Returns:
        Counts for ``gemlite``, ``fused``, ``packed``, ``eager`` and ``skipped`` layers.
    """
    ...
    with safe_open(str(checkpoint_path), framework="pt", device="cpu") as f:
        md = f.metadata() or {}
        if md.get("official_multibranch") != "true":
            raise ValueError(f"{checkpoint_path} is not an official multibranch checkpoint")
        ...
        for idx, raw_entry in enumerate(quant_layers, start=1):
            ...
            try:
                ...
                parent, child = _resolve_module(modules, name)
            except KeyError:
                if strict:
                    raise
                counts["skipped"] += 1
            ...
            if progress and (idx == 1 or idx % 50 == 0 or idx == len(quant_layers)):
                ...
                print(
                    f"[int4] loaded {idx}/{len(quant_layers)} layers "
                ...
                cuda_memory_summary(f"after int4 {idx}")
...
def load_qep_int4_into_official_net(*args: Any, **kwargs: Any) -> dict[str, int]:
    """Backward-compatible alias for earlier experiment scripts."""
    return load_int4_official_multibranch_into_net(*args, **kwargs)
